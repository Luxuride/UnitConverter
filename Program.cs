                ...
                try {
                    if(double.TryParse(values[0], out double number)) {
                        ...
                        if(values[1].Split('/').Length == 2 && values[2].Split('/').Length == 2) {
                            ...
                            Console.WriteLine($"{values[0]} {values[1]} -> {converter.ConvertAdvanced(values1[0], values2[0], values1[1], values2[1], number)} {values[2]}");
                        } 
                        // Convert simple value
                        else {
                            Console.WriteLine($"{values[0]} {values[1]} -> {converter.ConvertSimple(values[1], values[2], number)} {values[2]}");
                        }
                    }
                } catch(Exception ex) {
